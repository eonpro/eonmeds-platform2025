  ...
  const loadSavedCards = async () => {
    try {
      ...
      const response = await apiClient.get(
        `/api/v1/payments/patients/${invoice.patient_id}/cards`,
      );
  ...
  if (paymentSucceeded) {
    return (
      <div className="modal-overlay" onClick={onClose}>
        <div
          className="payment-modal-content success-state"
        ...
        <div className="invoice-summary">
          ...
          <div className="invoice-totals">
            <div className="total-row">
              <span>Subtotal</span>
              <span>
                {formatCurrency(invoice.total_amount || invoice.amount_due)}
              </span>
            </div>
        ...
        {paymentMethod === "saved" && savedCards.length > 0 && (
          <div className="modal-actions">
            ...
            >
              {processing
                ? "Processing..."
                : `Charge ${formatCurrency(invoice.amount_due)}`}
            </button>
          </div>
        )}
      </div>
