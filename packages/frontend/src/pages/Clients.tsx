...
export const Clients: React.FC = () => {
  ...
  const [newPatientNotification, setNewPatientNotification] = useState<
    string | null
  >(null);
  ...
  const fetchPatients = useCallback(
    async (search?: string, showLoading = true) => {
      if (!apiClient) return;
      ...
      try {
        ...
        console.log(
          `Fetched ${newPatients.length} patients at ${new Date().toLocaleTimeString()}`,
        );

        // Check for new patients (only when not loading initially)
        if (
          !showLoading &&
          newPatients.length > previousPatientCountRef.current
        ) {
          const diff = newPatients.length - previousPatientCountRef.current;
          setNewPatientNotification(
            `${diff} new patient${diff > 1 ? "s" : ""} added!`,
          );
  ...
  useEffect(() => {
    ...
    patients.forEach((patient) => {
      if (
        ...
        Array.isArray(patient.membership_hashtags)
      ) {
    ...
    <div className="clients-page">
      ...
      <div className="clients-controls">
        ...
        <div className="filter-section">
          ...
          <button className="add-new-client-btn" onClick={handleAddNewClient}>
            Add New Client
          </button>
        </div>
      </div>
              ...
              {displayedPatients.map((patient) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td>
                      {patient.membership_hashtags &&
                      patient.membership_hashtags.length > 0 ? (
