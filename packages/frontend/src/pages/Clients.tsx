...
export const Clients: React.FC = () => {
  ...
  const displayedPatients = useMemo(() => {
    ...
    return patients.filter(
      (patient) => patient.membership_hashtags && patient.membership_hashtags.includes(selectedTag)
    );
              ...
              {displayedPatients.map((patient) => {
                ...
                return (
                  ...
                  >
                    ...
                    <td>{patient.email}</td>
                    <td>
                      <span className="phone-single-line" title={patient.phone || ''}>
                      ...
                      {patient.membership_hashtags && patient.membership_hashtags.length > 0 ? (
                        <div className="tag-list">
                          {patient.membership_hashtags.map((tag) => (
                            <span key={tag} className="tag" data-tag-type={getHashtagType(tag)}>
                              {formatHashtagDisplay(tag)}
                            </span>
                          ))}
                        </div>
