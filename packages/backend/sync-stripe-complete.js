...
async function findOrCreatePatient(customer) {
  try {
    ...
    console.log(
      `   🆕 Created new patient: ${firstName} ${lastName} (${patientId})`,
    );
...
async function updatePatientStripeInfo(patientId, stripeCustomerId) {
  try {
    ...
    console.error(
      `Error updating Stripe info for patient ${patientId}:`,
      error,
    );
  }
}
...
async function updatePatientSubscriptionStatus(
  ...
  try {
    ...
    console.error(
      `Error updating subscription status for patient ${patientId}:`,
      error,
    );
  }
}
    ...
    for (const customer of customers) {
      try {
        ...
        if (successfulCharges.length > 0) {
          stats.withPayments++;
          console.log(
            `   💳 Has ${successfulCharges.length} successful payment(s)`,
          );

          // Calculate total paid
          const totalPaid =
            successfulCharges.reduce((sum, charge) => sum + charge.amount, 0) /
            100;
        ...
        if (activeSubscriptions.length > 0) {
          ...
          console.log(
            `   📅 Has ${activeSubscriptions.length} active subscription(s)`,
          );
