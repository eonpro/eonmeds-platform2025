  ...
  try {
    ...
    try {
      ...
      if (hasData) {
        ...
        console.log('Since the patient_id column is UUID and we need VARCHAR, we need to handle this carefully.\n');
        ...
        matchCheck.rows.forEach(row => {
          console.log(`- SOAP ID: ${row.id}, Patient UUID: ${row.soap_patient_uuid}`);
        });
      ...
      console.log('The patient_id column now uses VARCHAR(50) to match the patients table.');
