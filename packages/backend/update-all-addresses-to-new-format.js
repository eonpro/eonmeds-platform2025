    ...
    for (const patient of result.rows) {
      ...
      if (patient.address && !patient.address_house) {
        ...
        if (parsed.house || parsed.street) {
          ...
          console.log(`   → House: ${parsed.house}, Street: ${parsed.street}${parsed.apartment ? ', Apt: ' + parsed.apartment : ''}`);
          updatedCount++;
        } else {
          console.log(`⚠️  Could not parse address for ${patient.patient_id}: ${patient.address}`);
        }
      }
    }
...
function parseAddress(address) {
  ...
  let apartment = null;
  const aptMatch = address.match(/[,\s]+(apt|apartment|unit|#)\s*([a-zA-Z0-9-]+)/i);
  if (aptMatch) {
