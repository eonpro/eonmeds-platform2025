  ...
  submission: {
    ...
    fields: {
      ...
      previous_weight_loss_attempts:
        "Diet and exercise, but struggled with consistency",
...
async function testWebhook() {
  try {
    ...
    const webhookSecret =
      process.env.HEYFLOW_WEBHOOK_SECRET || "test-webhook-secret";
    const signature = generateSignature(testPayload, webhookSecret);

    console.log(
      "Sending test webhook to: http://localhost:3002/api/v1/webhooks/heyflow",
    );
