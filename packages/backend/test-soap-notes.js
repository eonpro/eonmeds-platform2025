...
async function testSOAPNotes() {
  ...
  try {
    ...
    try {
      const result = await client.query('SELECT * FROM soap_notes WHERE patient_id = $1', [
        'P0244',
      ]);
    ...
    if (patientCheck.rows.length > 0) {
      ...
      try {
        ...
        console.log('Insert successful! SOAP note ID:', insertResult.rows[0].id);

        // Clean up test data
        await client.query('DELETE FROM soap_notes WHERE id = $1', [insertResult.rows[0].id]);
