    ...
    if (invoiceResult.rows.length === 0) {
      ...
      if (invoiceByIdResult.rows.length === 0) {
        ...
        recentInvoices.rows.forEach(inv => {
          console.log(`  - ${inv.invoice_number}: ID=${inv.id}, Amount=$${inv.amount_due}, Status=${inv.status}`);
        });
      }
    } else {
      ...
      try {
        ...
        await client.query('DELETE FROM invoice_payments WHERE id = $1', [testPaymentResult.rows[0].id]);
