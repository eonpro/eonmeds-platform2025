...
async function checkRailwaySOAPNotes() {
  if (!RAILWAY_DATABASE_URL) {
    console.error(
      "❌ No Railway database URL found. Please set RAILWAY_DATABASE_URL environment variable.",
    );
    console.log(
      "You can find this in Railway dashboard > Your backend service > Variables > DATABASE_URL",
    );
    return;
  }
  ...
  try {
    ...
    if (tableCheck.rows[0].exists) {
      ...
      const patientIdCol = columnCheck.rows.find(
        (col) => col.column_name === "patient_id",
      );
      if (patientIdCol) {
        console.log(`\n✅ patient_id column type: ${patientIdCol.data_type}`);
        if (patientIdCol.data_type === "uuid") {
          console.log(
            "⚠️  WARNING: patient_id is UUID type, needs to be VARCHAR(50)",
          );
        }
      }
      ...
      if (fkCheck.rows.length > 0) {
        ...
        console.log(
          `- ${fkCheck.rows[0].constraint_name}: ${fkCheck.rows[0].column_name} -> ${fkCheck.rows[0].foreign_table_name}(${fkCheck.rows[0].foreign_column_name})`,
        );
      }
...
console.log(
  "This script checks the soap_notes table in your Railway database.\n",
);
console.log("To use this script:");
console.log("1. Get your Railway DATABASE_URL from the Railway dashboard");
console.log(
  '2. Run: RAILWAY_DATABASE_URL="your-railway-database-url" node check-railway-soap-notes.js\n',
);
...
if (RAILWAY_DATABASE_URL) {
  ...
  console.log(
    "❌ RAILWAY_DATABASE_URL not set. Please provide it to continue.",
  );
}
