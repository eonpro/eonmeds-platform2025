...
// Configuration
const WEBHOOK_URL =
  process.env.WEBHOOK_URL || 'http://localhost:3002/api/v1/payments/webhook/stripe';
...
async function runTests() {
  ...
  console.log(
    "SELECT patient_id, email, status, form_type, stripe_customer_id FROM patients WHERE email IN ('test@example.com', 'external@example.com') ORDER BY created_at DESC;"
  );

  console.log('\n2. Check webhook events:');
  console.log(
    "SELECT event_type, processed, error_message FROM webhook_events WHERE source = 'stripe' ORDER BY created_at DESC LIMIT 5;"
  );

  console.log('\n3. Check invoices:');
  console.log(
    'SELECT invoice_number, stripe_customer_id, status, total_amount FROM invoices ORDER BY created_at DESC LIMIT 5;'
  );
}
