...
export async function createRefund({
  ...
  try {
    ...
    await client.query(
      ...
      ['refund.created', JSON.stringify({ refund_id: refund.id, payment_intent_id, amount: refund.amount })]
    );
...
export async function createRefundByInvoice({
  ...
  try {
    ...
    await client.query(
      `INSERT INTO billing_events (id, type, payload, created_at) 
       VALUES (gen_random_uuid(), $1, $2, NOW())`,
      ['refund.created_by_invoice', JSON.stringify({ 
        refund_id: refund.id, 
        invoice_id, 
...
export async function createCreditNote({
  ...
  try {
    ...
    await client.query(
      `INSERT INTO billing_events (id, type, payload, created_at) 
       VALUES (gen_random_uuid(), $1, $2, NOW())`,
      ['credit_note.created', JSON.stringify({ 
