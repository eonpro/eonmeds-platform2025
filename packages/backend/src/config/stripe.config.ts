...
export const validateStripeConfig = () => {
  if (!stripeConfig.apiKey) {
    console.warn(
      "⚠️  STRIPE_SECRET_KEY not set - Stripe payment functionality will be disabled",
    );
    console.warn(
      "   Add STRIPE_SECRET_KEY=sk_test_... to your .env file to enable payments",
    );
    return false;
  }
  ...
  if (!stripeConfig.webhookSecret) {
    console.warn(
      "⚠️  STRIPE_WEBHOOK_SECRET not set - webhooks will not be verified",
    );
    console.warn(
      "   Add STRIPE_WEBHOOK_SECRET=whsec_... to your .env file for webhook security",
    );
  }
